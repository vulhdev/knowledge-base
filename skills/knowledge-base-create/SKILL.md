...
description: Save a spec, plan, or idea from the current conversation into the knowledge base database. Use when the user asks to save, store, or persist something just created in this conversation — e.g. "save this spec", "lưu plan này", "store the idea we just discussed".
---
...
Check the user's message for an explicit type keyword: `spec`, `plan`, or `idea`.
...
Call:
```
create_content(workspace=WORKSPACE, feature=FEATURE, type=TYPE, body=CONTENT)
```
