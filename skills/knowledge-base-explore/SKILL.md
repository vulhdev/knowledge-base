...
- Present a compact summary to orient yourself:
  ...
  - The index table (ID | Type | Summary)
...
Call `search_content` without a type filter:

```
search_content(query=<feature>, workspace=WORKSPACE, limit=10)
```

Present the results: feature name, type, content ID, and a short excerpt of the body.
...
- Quick mode returned a digest index but you need the **full body** of a specific spec or plan to implement something
- The user explicitly asks to "read the full spec" or "load the full plan"
...
Load only the IDs that are directly relevant to the current task — not all of them.
...
- [#12 · spec] Short excerpt from body...
- [#18 · plan] Short excerpt from body...
```
...
| Situation | Action |
|---|---|
| Digest found | Present TL;DR + index, stop (quick mode) |
| Digest found + need full content | Load specific IDs via `get_content` (deep mode) |
| No digest, search results found | Present raw search results |
...
→ `search_content(query="payment", type="digest")` → no digest → `search_content(query="payment webhook")` → present raw results → proceed
